...
def mvn_basis(
    ...
    * fi - [NxM] is the basis function vectors containing a basis function
    ...
    for i in range(N):
        for j in range(M):
            fi[i,j] = multivariate_normal.pdf(X[i,:], mean=mu[j,:], cov = sigma)
    return fi
...
def max_likelihood_linreg(
    ...
    * lamda: The regularization constant
    ...
    N,M = np.shape(fi)
    return np.matmul(
        np.matmul(np.linalg.inv(np.matmul(np.transpose(fi), fi)+lamda*np.identity(M)),np.transpose(fi)), targets)
...
_plot_mvn()
lamda = 0.001
print(max_likelihood_linreg(fi, t, lamda))
